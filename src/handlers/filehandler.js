...
async function CheckForDeletedFiles(folders) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    if ((await FileExits(file.fullpath)) == false || fileInKnowFolder == false) {
      ...
      if (await FileExits(file.fullpath)) {
...
function GenerateImageThumbnail(file) {
  //console.log('Generating thumbnail for ' + file.fullpath);
  ...
  sharp(file.fullpath)
    ...
    .toFile(thumbPath, (err) => {
      if (err) {
        console.error('Error creating thumbnail for ' + file.fullpath);
        console.error(err);
        let strerror = String(err);
        console.log('str error ' + strerror);

        file.thumberror = strerror;
        ...
        db.EditFile(file);
        console.log('Thumbnail created for ' + file.fullpath + ' Thumbnail path: ' + thumbPath);
      }
    });
}
